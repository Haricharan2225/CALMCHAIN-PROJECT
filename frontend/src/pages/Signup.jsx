...
export const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const role = email === "haripatel2225@gmail.com" ? "admin" : "user";
      const userCol = role === "admin" ? "admins" : "users";

      await setDoc(doc(db, userCol, user.uid), {
        email: user.email,
        role: role,
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
         ...
         <motion.div 
            ...
            className="absolute top-[30%] -right-[10%] w-[50%] h-[70%] bg-indigo-900/20 blur-[150px] rounded-full" 
         />
      </div>
      ...
      >
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-black text-white mb-2 tracking-tight">Join CalmChain</h1>
          <p className="text-slate-400 font-medium text-base">Start your journey to tranquility</p>
        </div>
        ...
        <p className="text-center mt-10 text-slate-500 text-sm font-bold">
          Already have secrets? <Link to="/login" className="text-cyan-400 font-black hover:text-cyan-300 transition underline decoration-2 underline-offset-8">Log in</Link>
        </p>
      </motion.div>
    </motion.div>
  );
